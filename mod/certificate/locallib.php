...
function certificate_get_issues($certificateid, $sort="ci.timecreated ASC", $groupmode, $cm, $page = 0, $perpage = 0) {
    ...
    $pic = \core_user\fields::for_userpic($context)->get_required_fields();
    $identity = \core_user\fields::for_identity($context)->get_required_fields();
    $ar = array_unique(array_merge($pic,$identity));
    $extrafields = implode(',', array_map(function($v) {
        return 'u.' . $v;
    }, $ar));
    $users = $DB->get_records_sql("SELECT {$extrafields}, u.idnumber, ci.code, ci.timecreated
                                     FROM {user} u
 ...
 */
function certificate_get_course_time($courseid, $excludeObjects = array()) {
    global $CFG, $DB, $USER;
    ...
    $filter = '';
    foreach($excludeObjects as $exclusion) {
        $filter .= " AND objecttable <> '$exclusion'";
    }
...
// accept a HH:MM:SS.mmm string and return the number of seconds (including milliseconds)
function certificate_TimeToSec($time) {
    $sec = 0;
...
function certificate_secondsToHhmmss($seconds) {
    return sprintf('%02d:%02d:%02d', ($seconds/ 3600),($seconds/ 60 % 60), $seconds% 60);
}

// some WEBINARS use lesson, some use scorm. So we need to know which routine to call to calculate time spent
// reverse the cm array to get the top-most module name, which is likely the right one
function certificate_calculate_course_module_type_for_timespent($course) {
    $name = '';
    $modinfo = get_fast_modinfo($course);
    foreach(array_reverse($modinfo->get_cms()) as $cm) {
        if ($cm->modname === 'scorm') $name = 'scorm';
        if ($cm->modname === 'lesson') $name = 'lesson';
    }
    return $name;
}

/**
 * THIS routine is the main entry point to calculating the time spent in the course that gets printed on the certificate
 * most courses work the same way that live in a particular category - webinars use lessons, e-learning courses use scorm, etc.
 * find out the correct method for calculating the time spent
 *
 * Some courses won't work this method as they use multiple activities
 * e.g. Cyber & Privacy - these need individual overrides
 *
 * CALLED BY
 *  /mod/certificate/type/2022/certificate.php        line 48
 */
function certificate_getCertificateTimeSpent($courseid, $userid) {
    ...
    switch (strtolower($category->name)) {
        case 'webinars': // total time spent in lesson activities
            $subtype = certificate_calculate_course_module_type_for_timespent($courseid);
            switch ($subtype) {
                case 'lesson':
                    // prioritise poll-based timestamping
                    $total = certificate_getLessonPolledTime($courseid, $userid);
                    if ($total == 0) { // but fall back to lesson based times if that didn't work
                        $total = certificate_getLessonTotalTime($courseid, $userid);
                    }
                
                break;
                case 'scorm':
                    $total = certificate_getScormTotalTime($courseid);
        ...
        case 'elearning courses': // total time spent in scorm activities
            $total = certificate_getScormTotalTime($courseid);
            break;

        case 'handbooks': // total time spent in scorm
            $total = certificate_getScormTotalTime($courseid);
            break;

        case 'videos': // no completions, would be time spent in h5p activities
            break;

        case 'racgp alm': // total time spent in course
            $total = certificate_getCourseTotalTime($courseid);
            break;
...
function certificate_get_polled_time($cm, $userid) {
global $DB;

    $context = context_module::instance($cm->id);
    $time = 0;

    $rec = $DB->get_record('poll_events', ['userid'=>$userid,'contextid'=>$context->id], '*', IGNORE_MISSING);
    if ($rec) {
        $time = intval($rec->lastupdated) - intval($rec->begun);
    }
    return $time;
}

// 2023-03-29 Tim: Hopefully there is only one lesson; if not we will take the longest
function certificate_getLessonPolledTime($courseid, $userid) {
global $DB, $CFG;
    $cms = get_coursemodules_in_course('lesson', $courseid);
    $polls = [];
    $total = 0;
    foreach ($cms as $cm) {
        $polls[] = certificate_get_polled_time($cm, $userid);
    }
    if (!empty($polls)) $total = max($polls);
    return $total;
}

// 2022-08-03 Tim: Calculating lesson time spent, assuming one lesson per course.
function certificate_getLessonTotalTime($courseid, $userid) {
global $DB, $CFG;
    require_once($CFG->dirroot . '/mod/lesson/locallib.php');

    // need the course record for the lesson object
    $course = get_course($courseid);

    // we are here ASSUMING that one lesson per course is being used
    // then find the LONGEST attempt
    $cms = get_coursemodules_in_course('lesson', $course->id);
    $total = 0;
    $polled_totals = [];
    foreach ($cms as $cm) {
        // $polled_totals[] = certificate_get_polled_time($cm, $userid);
        $lesson = new lesson($DB->get_record('lesson', array('id' => $cm->instance), '*', MUST_EXIST), $cm, $course);


        // there is no routine to get all lesson attemps for one user
        // you have to report on all users then parse the data for the attempts of the user you want
        list($table, $data) = lesson_get_overview_report_table_and_data($lesson, false);

        $longest = 0;
        foreach ($data->students as $student) {
            if (strcmp($student->id, $userid) == 0) { // might be int or string, so compare as string
                foreach ($student->attempts as $attempt) {

                    // look for the COMPLETED attempts (completed means has a grade)
                    if (!is_null($attempt["grade"])) {
                        $length = $attempt["timeend"] - $attempt["timestart"];
                        if ($length > $longest) {
                            $longest = $length;
                        }
                    }
        ...
        // should only count one lesson, but will try to support this if it happens
        if ($longest > $total) {
            $total = $longest;
        }
    }

    // // if total was zero for some reason, use the time reported by the polling data
    // $polled_sum = array_sum($polled_totals);
    // if ($polled_sum > 0 && $total == 0) {
    //     $total = $polled_sum;
    // }
...
function certificate_getCourseTotalTime($courseid) {
global $DB, $USER;
...
function certificate_getScormTotalTime($courseid) {
global $DB, $USER;
    $tracks = $DB->get_records('scorm', array('course' => $courseid), 'id,version');

    $total = 0;
    $polled_totals = [];
    foreach ($tracks as $track) {
        $cm = $DB->get_record('course_modules',['instance'=>$track->id,'course'=>$courseid], 'id', MUST_EXIST);
        $polled_totals[] = certificate_get_polled_time($cm, $USER->id);

        if ($track->version == 'SCORM_1.2') {
            $rec = $DB->get_record("scorm_scoes_track", array('userid' => $USER->id, 'scormid' => $track->id, 'element' => 'cmi.core.total_time'),'value');
            if ($rec) {
                $total += certificate_TimeToSec($rec->value); // strtotime($rec->value) - strtotime('TODAY');
            }
        } else {
            $rec = $DB->get_record("scorm_scoes_track", array('userid' => $USER->id, 'scormid' => $track->id, 'element' => 'cmi.total_time'));
            $di = new DateInterval(preg_replace('/\.\d{2}/','',$rec->value)); // remove the fractional seconds as dateinterval doesn't support them
            $ref = new DateTimeImmutable();
            $end = $ref->add($di);
            $total += $end->getTimestamp() - $ref->getTimestamp();
        }
    }
...
function certificate_is_roleswitched() {
...
// accept a parsable date string
// such as '7 minutes 5 seconds'
// or '03:23:12.15'
// and return the number of seconds
// function secondsInTime($timestring) {
//     $d = new DateTime('1970-1-1');
//     $d->modify($timestring);
//     sscanf($d->format("H:i:s"), "%d:%d:%d", $hours, $minutes, $seconds);
//     return isset($seconds) ? $hours * 3600 + $minutes * 60 + $seconds : $hours * 60 + $minutes;
// }

// function getScormTimeSpent($courseid, $userid) {
// global $DB;

//     // find all scorm activities in this course (e.g. cyber and privacy has 4 scorm activities)

//     $tracks = $DB->get_records('scorm', array('course' => $courseid), 'id');
//     $diff = 0;
//     foreach ($tracks as $scormid) {
//     // look up FIRST x.start.time and get the 'timemodified' e.g. 1652243321
//         $first = $DB->get_record_sql("
//             select timemodified from {scorm_scoes_track}
//             where element = 'x.start.time'
//             and userid = :userid
//             and scormid = :scormid
//             group by userid,scormid
//             order by attempt asc
//         ", array('userid' => $userid, 'scormid' => $scormid->id));
//         $last = $DB->get_record_sql("
//             select timemodified from {scorm_scoes_track}
//             where (element like 'cmi.%exit' and value = '')
//             and userid = :userid
//             and scormid = :scormid
//             group by userid,scormid
...
//         ", array('userid' => $userid, 'scormid' => $scormid->id));
//         if ($first && $last) {
//             $diff += $last->timemodified - $first->timemodified;
//         }
//     }
//     return $diff;
// }
