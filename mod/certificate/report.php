...
// foreach ($extrafields as $field) { echo "<li>EXTRAFIELD: " . $field . "</li>"; }

$extrafields = \core_user\fields::for_identity($context)->get_required_fields();
// foreach ($extrafields as $field) { echo "<li>IDENTITY: " . $field . "</li>"; }
...
// $usercount = count(certificate_get_issues($certificate->id, $sort, $groupmode, $cm));
...
foreach ($users as $user) {
    $name = fullname($user); //  $OUTPUT->user_picture($user) . fullname($user);
