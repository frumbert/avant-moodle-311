...
class auth_plugin_token extends auth_plugin_base
{
    ...
    // record the licence plate of the horse they rode in on
    public function user_authenticated_hook(&$user, $username, $password) {
        set_user_preference('useragent', $_SERVER['HTTP_USER_AGENT'], $user);
    }
    ...
    function user_login($username, $password) {
        global $CFG, $DB;
        if (($user = $DB->get_record('user', array('username' => $username, 'mnethostid' => $CFG->mnet_localhost_id))) !== false) {
    ...
    public function loginpage_hook() {
        ...
        // if data is submitted we hit /login/index.php with a form post
        if (data_submitted()) {
            $CFG->alternateloginurl = '/auth/token/login.php'; // Don't allow redirect; you might need to see an error status
        }
    }
