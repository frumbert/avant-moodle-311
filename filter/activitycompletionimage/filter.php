...
// var_dump(self::$activitylist);
//        $foundLinks = array_keys(self::$activitylist);
        libxml_use_internal_errors(true); // 2021 Tim https://stackoverflow.com/a/11819635/1238884 handle invalid xhtml
        $dom = new DOMDocument();
        ...
        libxml_clear_errors(); // cleanup error buffer, we no longer care
        $hrefs = $dom->getElementsByTagName("a");
        foreach ($hrefs as $href) {
            $hrefkey = trim($href->getAttribute("href"));
            // echo "Looking for <code>-", bin2hex($hrefkey), "-</code>";
            if (!array_key_exists($hrefkey, self::$activitylist)) {
                // echo " was not found in list";
                continue;
            }
            $info = self::$activitylist[$hrefkey]; // find this <a href> in the activity list
