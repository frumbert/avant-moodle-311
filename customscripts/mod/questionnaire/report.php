...
switch ($action) {
    ...
    case 'dfs':
        ...
        // $name .= '_' . userdate(time(), '%Y%m%d%H%M');
        $name = preg_replace("/[^A-Z0-9_]+/i", "_", trim($name));

        switch ($name) {
            case "Avant_Assist_Evaluation_Survey":
            case "Avant_Assist_Self_Assessment":
                switch ($COURSE->idnumber) {
                    case "aa_gp_eval":
                    case "aa_gp_anon":
                        $name .= "_gp";
                        break;

                    case "aa_surgeon_eval":
                    case "aa_surgeon_anon":
                        $name .= "_surgeon";
                        break;
                }
                break;
        }
        $name .= '_' . date_format(date_create("now"),"YmdHis");
    ...
    case 'vallarsort':   // View all responses sorted in descending order.
        ...
        if ($outputtarget == 'pdf') {
            $pdf = customscript_questionnaire_report_start_pdf();
            if ($currentgroupid > 0) {
                $groupname = get_string('group') . ': <strong>' . groups_get_group_name($currentgroupid) . '</strong>';
            } else {
                $groupname = '<strong>' . get_string('allparticipants') . '</strong>';
 ...
 * @return pdf
 */
function customscript_questionnaire_report_start_pdf() {
    global $CFG;
