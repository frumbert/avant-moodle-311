...
// $opoverride = optional_param('opoverride',0,PARAM_INT);set_user_preferences(['opoverride'=>$opoverride===1?'true':'false']);
...
// Avant Assist don't want the word 'Course' in the header; the rest of the site can adjust it in the language strings

    if ($section and $section > 0 and course_format_uses_sections($course->format)) {
        $sectionname = get_string('sectionname', "format_$course->format");
        $sectiontitle = get_section_name($course, $section);
        if ($PAGE->theme->name === "classicrenderer") {
            $PAGE->set_title("{$course->fullname}: {$sectiontitle}"); //, {$sectionname}");
        } else {
    ...
    } elseif ($PAGE->theme->name === "classicrenderer") {
