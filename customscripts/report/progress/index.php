...
if ($csv && $grandtotal && count($activities)>0) { // Only show CSV if there are some users/actvs
    ...
    if (strpos($course->idnumber, "aa_") === false) {
        $shortname = format_string($course->shortname, true, array('context' => $context));
        header('Content-Disposition: attachment; filename=progress.' .
            preg_replace('/[^a-z0-9-]/','_',core_text::strtolower(strip_tags($shortname))).'.csv');
    } else {
        $speciality = str_replace('aa','', $course->idnumber);
        $datetime = date_format(date_create("now"),"YmdHis");
        $filename = "Avant_Assist_Engagement_Overview_{$speciality}_{$datetime}.csv";
...
if (!$csv) {
    ...
    foreach ($extrafields as $field) {
        if ($field === "memberid") {
            echo '<th scope="col" class="completion-identifyfield">Member Id</th>';
        } else {
    ...
    echo csv_quote(get_string('fullname'));
    foreach ($extrafields as $field) {
        if ($field === "memberid") {
            echo $sep . csv_quote('Member ID');
        } else {
...
foreach($progress as $user) {
    ...
    if ($csv) {
        ...
        foreach ($extrafields as $field) {
            if ($field === "memberid") {
                echo $sep . csv_quote(get_user_preferences('memberid','',$user));
            } else {
        ...
        foreach ($extrafields as $field) {
            if ($field === "memberid") {
                echo '<td>' . get_user_preferences('memberid','',$user) . '</td>';
            } else {
